"""CLI tests for `vibedft qe relax review`."""
...
def _write_relax_text(path: Path, *, status: str) -> None:
    if status == "pass":
...
convergence has not been achieved after   2 iterations
JOB END.
"""
...
def test_qe_relax_review_cli_pass(tmp_path: Path) -> None:
    output_file = tmp_path / "relax_pass.out"
    _write_relax_text(output_file, status="pass")

    payload_text = _run_cli(["qe", "relax", "review", str(output_file)])
    payload = json.loads(payload_text)

    assert payload["command"] == "qe.relax.review"
    assert payload["ok"] is True
    assert payload["result"]["status"] == "pass"
    assert payload["result"]["task"] == "relax"
    assert payload["result"]["review"]["status"] == "PASS"
    assert payload["result"]["readiness"]["downstream"]["scf"]["allowed"] is True


def test_qe_relax_review_cli_block_and_fail_on_block(tmp_path: Path) -> None:
    output_file = tmp_path / "relax_block.out"
    _write_relax_text(output_file, status="block")
    ...
    failure_output = _run_cli(
        ["qe", "relax", "review", "--fail-on-block", str(output_file)],
        expect_exit_code=2,
    )
    failure_payload = json.loads(failure_output)

    assert failure_payload["command"] == "qe.relax.review"
    assert failure_payload["ok"] is True
    assert failure_payload["result"]["status"] == "block"


def test_qe_relax_review_cli_output_file(tmp_path: Path) -> None:
    output_file = tmp_path / "relax_pass.out"
    json_file = tmp_path / "relax_review.json"
    _write_relax_text(output_file, status="pass")
    ...
    assert file_payload["result"]["task"] == payload["result"]["task"] == "relax"
