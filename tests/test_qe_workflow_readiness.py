...
from vibedft.calculator.qe.observability import build_workflow_readiness_graph_from_parsed
...
def test_workflow_graph_complete_scf_and_phonon_is_followup_ready() -> None:
    ...
    graph = build_workflow_readiness_graph_from_parsed(scf_output=scf, phonon_output=ph)

    assert graph.scf_stage.status == "complete"
    assert graph.scf_stage.ready_for_followup is True
    assert graph.phonon_stage.status == "warn"
    ...
    assert any("negative frequencies" in blocker for blocker in graph.blockers)
    assert graph.to_schema()["stages"]["phonon"]["status"] == "warn"
...
def test_workflow_graph_running_scf_blocks_followup_and_keeps_actions() -> None:
    ...
    graph = build_workflow_readiness_graph_from_parsed(scf_output=scf, phonon_output=None)
